...
const SectionListSample = () => {
  ...
  products.forEach((item) => {
    let sectionItem = data.find((q) => q.prime == item.userId);

    if (sectionItem) {
      sectionItem.data.push(item.id,item.title, item.completed);
    } else {
      let itemSample = {
        prime: item.userId,
        data: [item.id, item.title, item.completed],
      };
  ...
  return (
    <>
      <SectionList
        sections={data}
        renderItem={Box}
        renderSectionHeader={({ section: { prime } }) => (
          <Text style={{ fontSize: 90 }}>{prime}</Text>
        )}
      />
    </>
  );
